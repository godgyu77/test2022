...
public class EmpUpdateController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		// json Type이라 에러 발생한것 빈 공간이라도 json 규칙에 따라 파싱하려고 노력하기때문에
		/* resp.setContentType("application/json"); */
		resp.setCharacterEncoding("UTF-8");
		String[] param = { req.getParameter("empno"), req.getParameter("ename"), req.getParameter("sal") };
		try (Connection conn = BitMy.getConnection(); Statement stmt = conn.createStatement();) {
